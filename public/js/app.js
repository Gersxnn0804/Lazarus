...
async function loadData() {
  try {
    const response = await fetch('/api/lazarus-data', { cache: 'no-store' });
    const payload = await response.json();

    if (!response.ok || !payload.ok || !payload.hasData) {
      resetDashboard(payload.message || 'No se pudo leer el JSON.');
      return;
    }
    ...
    resetDashboard('No fue posible conectar con el servidor local.');
    console.error(error);
  }
}
